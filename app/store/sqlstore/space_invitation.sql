...
-- 添加字段注释
COMMENT ON COLUMN bw_invitation.id IS '主键ID';
COMMENT ON COLUMN bw_invitation.inviter_id IS '邀请人ID';
COMMENT ON COLUMN bw_invitation.invitee_email IS '被邀请人邮箱';
COMMENT ON COLUMN bw_invitation.space_id IS '邀请所属的空间ID';
COMMENT ON COLUMN bw_invitation.role IS '被邀请人权限（如：admin, member, guest）';
COMMENT ON COLUMN bw_invitation.invite_status IS '邀请状态（1=待接受，2=已接受，3=已拒绝，4=已过期）';
COMMENT ON COLUMN bw_invitation.created_at IS '邀请创建时间';
COMMENT ON COLUMN bw_invitation.expired_at IS '邀请过期时间';
COMMENT ON COLUMN bw_invitation.updated_at IS '最后更新时间';
