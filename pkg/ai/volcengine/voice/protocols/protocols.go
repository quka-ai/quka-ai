...
const (
	...
	EventType_PodcastEnd           EventType = 363
...
// GzipCompressData compresses input data using gzip
func GzipCompressData(input []byte) []byte {
	var b bytes.Buffer
	w := gzip.NewWriter(&b)
	w.Write(input)
	w.Close()
...
// GzipDecompressData decompresses gzipped input data
func GzipDecompressData(input []byte) []byte {
